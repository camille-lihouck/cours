...
def create (n,f,m):
    ...
    :rtype: dict
    """
    return {'table':[False for i in range(2**n)],'hash_f':f ,'num_f':m }
...
def add (bf, e):
    ...
    :type e: Any
    """
    for i in range (bf['num_f']):
        bf['table'][bf['hash_f'](e,i)%len(bf['table'])]= True
...
def contains (bf, e):
    ...
    """
    for i in range (bf['num_f']):
        if (bf['table'][bf['hash_f'](e,i)%len(bf['table'])]!= True):
            return False
    return True
